...
export default function VoterTable({ votes }: VoterTableTypes) {
  ...
  return (
    <motion.div
      layout
      className="mb-6 border-b border-dashed pb-6 border-gray-700"
    >
      <table
        {...getTableProps()}
          ...
          {page.map((row, idx) => {
            ...
            return (
              <tr
                {...row.getRowProps()}
                key={idx}
                className="mb-3 grid border-b pb-3 border-gray-700 sm:mb-0 sm:table-row sm:border-b-0 sm:pb-0"
              >
